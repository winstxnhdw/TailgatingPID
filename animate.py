...
class Car:
    ...
    def drive(self, target_x, target_y):
        ...
        self.throttle = velocity_control(self.v, self.gap, self.prev_gap, self.safety_thresh, self.dt)
        self.tracker = PathTracker(self.k, self.ksoft, self.max_steer, self.L, self.throttle, self.x, self.y, self.yaw, self.px, self.py, self.pyaw)
        self.throttle, self.delta = self.tracker.stanley_control()
        self.kbm = KinematicBicycleModel(self.x, self.y, self.yaw, self.v, self.throttle, self.delta, self.L, self.max_steer, self.dt, self.c_r, self.c_a)
        self.x, self.y, self.yaw, self.v, self.delta, self.omega = self.kbm.kinematic_model()
...
def main():
    ...
    annotation = ax[0].annotate('{}'.format(np.around(car.gap, 2)), xy=(car.x, car.y + 5), color='black', annotation_clip=False)
    ...
    target_throttle = np.abs(signal.sawtooth(2*np.pi*2*path.px)) * 1000
    ...
    def animate(frame):
        ...
        target.throttle = target_throttle[frame]
        target.drive()
        outline_plot_t, fr_plot_t, rr_plot_t, fl_plot_t, rl_plot_t = desc.plot_car(target.x, target.y, target.yaw, target.delta)
        ...
        annotation.set_text('Gap: {} m'.format(np.around(car.gap, 2)))
        annotation.set_position((car.x, car.y + 3))
